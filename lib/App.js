...
const { getRandomChar } = require('./shared');
...
const USE_HELP_COMMAND = `Benutz \`${ PREFIX } help\` für weitere Informationen`;
...
class Command {
	...
	toString() {
		const usage = `${PREFIX} ${ this.name } ${ this.args }`;
		return `${ markdownCode(usage) } - ${ this.description }`;
	}
}
...
module.exports = class App {
	...
	onBotCommandMessage(message) {
		...
		if (!command) {
			message.channel.send(`Unbekannter Befehl: ${ markdownCode(commandName) } mit den Argumenten: ${ markdownCode(args.join(' ')) }! ${ USE_HELP_COMMAND }`);
			return;
		}
	...
	callNext() {
		...
		this.sleepTimeout = setTimeout(function () {
			...
			for (let i = 0; i < self.currentLength; i++) {
				self.currentString += getRandomChar();
			}
